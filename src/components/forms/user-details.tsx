  ...
  const onChangePermission = async (
    ...
    if (type === 'agency') {
      await saveActivityLogsNotification({
        ...
        description: `${userData?.name} дан доступ к | ${
  ...
  const onSubmit = async (values: z.infer<typeof userDataSchema>) => {
    ...
    if (userData || data?.user) {
      ...
      authUserData?.Agency?.SubAccount.filter((subacc) =>
        ...
        await saveActivityLogsNotification({
          agencyId: undefined,
          description: `Информация ${userData?.name} обновлена`,
      ...
      if (updatedUser) {
        ...
        toast({
          variant: 'destructive',
          title: 'Упс!',
          description: 'Не получилось обновить пользовательскую информацию',
        })
      }
    } else {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Добавьте или обновите свою информацию</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      if (
                        ...
                        setRoleState(
                          'Чтобы назначить доступ к субаккаунту членам команды, необходимо иметь субаккаунты.'
                        )
                      } else {
                        setRoleState('')
                      }
            ...
            {authUserData?.role === 'AGENCY_OWNER' && (
              <div>
                ...
                <FormDescription className="mb-4">
                  Вы можете предоставить доступ к субаккаунту члену команды, включив
                  контроль доступа для каждой субаккаунта. Это видно только для
                  владельцы агентств
                </FormDescription>
