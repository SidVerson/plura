...
const LaneForm: React.FC<CreateLaneFormProps> = ({
  ...
  const onSubmit = async (values: z.infer<typeof LaneFormSchema>) => {
    ...
    try {
      ...
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Обновлен коридор | ${response?.name}`,
        subaccountId: d.subAccountId,
      })

      toast({
        title: 'Успех',
        description: 'Детали пайплайна сохранены',
      })
      ...
      toast({
        variant: 'destructive',
        title: 'Ошибка!',
        description: 'Не удалось сохранить детали пайплайна',
      })
    }
    setClose()
  }
  return (
      <Card className="w-full ">
        <CardHeader>
          <CardTitle>Детали коридора</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form
            ...
            >
              <FormField
                  ...
                  render={({ field }) => (
                      <FormItem>
                        <FormLabel>Название коридора</FormLabel>
                        <FormControl>
                          <Input
                              placeholder="Название коридора"
                              {...field}
                          />
                        </FormControl>
                        <FormMessage />
